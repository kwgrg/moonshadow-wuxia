...
for(let revision=1;revision<=12;revision++)for(const numeric of [false,true]){
 ...
 for(const id of ['g13','g14','g15']){const raw=historical(revision,id,{numeric,done:true}),g=new GameEngine(restoreState(raw));preservedHistory(g,raw);assert.equal(g.s.flags.goodForbiddenLegacy,true);assert.equal(g.q.id,id==='g13'?'g14_dock_report':id==='g14'&&revision<12?'g15':id);}
