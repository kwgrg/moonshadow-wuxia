...
function option(g,outcome){const id=g.q.id;if(id==='g15')return outcome==='cult'?0:1;if(id==='g20')return outcome==='zhen_good'?1:0;if(id==='g23')return outcome==='three'?1:0;if(id[0]==='e'){const high=outcome==='alone';return {e02:high?1:0,e04:high?1:0,e06:high?1:0,e08:high?1:0,e09:high?0:1}[id]??0;}return ['alone','family'].includes(outcome)?g.q.choice.options.length-1:0;}
for(const difficulty of ['normal','story']) for(const outcome of ['reunion','three','zhen_good','cult','alone','family']){
 ...
 while(!g.s.completed&&transitions++<1000){const q=g.q;
  ...
  if(g.s.phase==='battle'){
   battles++;let ticks=0;
   if(q.scriptedLoss){g.s.hero.x=800;g.s.hero.y=650;while(g.s.phase==='battle'&&ticks++<3000)g.tick(.05);}
   else while(g.s.phase==='battle'&&ticks++<6000&&!g.paused){
  ...
  if(g.s.phase==='search'){let safety=0;while(g.s.phase==='search'&&safety++<40){const marker=g.markers.find(m=>m.main);assert.ok(marker,q.id);g.interact(marker);for(let t=0;t<500&&g.autoInteract;t++)g.tick(.05);assert.equal(g.autoInteract,null,`${q.id} pathfinding reaches collectible`);}assert.equal(g.s.phase,'after',q.id);}
...
const opening=new GameEngine();opening.s.quest=QUESTS.findIndex(q=>q.id==='a05');opening.s.map=opening.q.map;opening.startBattle();opening.s.enemies[0].hp=1;opening.s.hero.x=opening.s.enemies[0].x;opening.s.hero.y=opening.s.enemies[0].y;opening.cast(0);assert.equal(opening.s.ending,'opening');
...
path.keys.add('ArrowLeft');for(let i=0;i<2000;i++)path.tick(.05);assert.ok(path.s.hero.x>=410);path.keys.clear();assert.equal(path.equipSkill(18,0),false);path.unlock(18);assert.equal(path.equipSkill(18,4),true);
...
console.log(JSON.stringify({result:'PASS',questNodes:QUESTS.length,regions:Object.keys(MAPS).length,skills:SKILLS.length-1,endings:7,checks:'All six long-route outcomes, opening branch, collection/pathfinding, 12 herbs, 8/5 tower floors, defeat events, storage, skill equip, donations and seven fragments',campaigns:stats},null,2));
