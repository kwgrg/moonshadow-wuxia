...
if(!process.argv.includes('--migration-only')){
 for(const answer of [0,1]){
  ...
  assert.equal(g.q.id,'e05');assert.deepEqual(g.s.flags.companions,[],'new party ledger agrees with the completed departure');assert.ok(!g.s.flags.cultPath&&!g.s.flags.evilQiangweiDead);const changed=Object.keys(g.s.flags).filter(key=>g.s.flags[key]!==beforeFlags[key]);assert.ok(changed.every(key=>['companion','companions'].includes(key)||milestones.includes(key)||ids.some(id=>key==='staged_'+id)),'return chapter cannot invent a marriage, death or unrelated branch: '+changed.join(','));
