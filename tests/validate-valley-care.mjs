...
assert.equal(freshState().campaignRevision,12);assert.ok(Array.isArray(oldTables[8]));assert.ok(!oldTables[8].includes('g06_confide'));
...
for(const [oldIndex,id] of oldTables[8].entries()){
 ...
 const restored=restoreState(raw);assert.equal(QUESTS[restored.quest].id,({e06_rest:'e06_first_interlude',e05:'e04_homecoming',g14:'g14_dock_report'})[id]||id,id+' keeps its old numeric identity or an explicit new unfinished-stage migration');assert.equal(QUESTS[index(id)].encounterTier,Math.max(1,Math.floor(oldIndex/9)+1),id+' keeps its battle tier');legacyCases++;
}
