...
assert.equal(freshState().campaignRevision,10);assert.ok(Array.isArray(oldTables[8]));assert.ok(!oldTables[8].includes('g06_confide'));
...
for(const [oldIndex,id] of oldTables[8].entries()){
 ...
 const restored=restoreState(raw);assert.equal(QUESTS[restored.quest].id,id,id+' keeps its old numeric identity');assert.equal(QUESTS[index(id)].encounterTier,Math.max(1,Math.floor(oldIndex/9)+1),id+' keeps its battle tier');legacyCases++;
}
