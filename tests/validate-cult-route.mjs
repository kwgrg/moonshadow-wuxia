...
for(const [revision,ids] of [[1,LEGACY_QUEST_IDS],[2,REVISION_TWO_QUEST_IDS],[3,REVISION_THREE_QUEST_IDS]])for(const id of ['g15','g16','e07','e14']){
 ...
 const restored=restoreState(raw);assert.equal(QUESTS[restored.quest].id,id,'legacy index keeps its quest identity');assert.equal(restored.campaignRevision,7);migratedIndices++;
