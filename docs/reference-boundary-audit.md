...
## 2026-09-24 R16只读参考增量

本轮只保留求医2030至终局2041的机制概述和定位元数据（good-medicine-reunion-reference.md），未把原脚本正文、完整对白、原布局坐标或游戏媒体带入仓库。新增medicine-courtyard.png由内置image_gen只用文字生成，没有参考图输入；完整提示词、生成路径和交付SHA见medicine-courtyard-art.md，资源清单现40PNG。

新产品模块独立实现任务状态、两名实际首谈对象、单/双同行和旧存档迁移；演出与对白重新创作。扫描public没有原目录路径或LoadMap/AddGoods/DelGoods调用。既有source字符串里的包项定位不构成读取原包依赖。本轮没有建立原资源导入工具，也未以其他游戏引擎替代本项目。

静态配置继续只有public目录。HTTP通过104匿名游戏资源和13非发布路径，Wrangler dry-run读105文件且无绑定，不代表已上线。制作记录和一致性检查不等同法律认证；保留用户明确的参考而不复制边界。
