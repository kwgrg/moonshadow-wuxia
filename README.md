...
当前版本为存档修订12，包含204个剧情节点、99个现实地点、18门武学和7个结局；这些数量仅描述当前数据规模，不代表原作覆盖率。剧情按攻略与本地只读参考分阶段核对，衔接对白独立编写；已验证范围见审计文档，不表示原版逐句一致。部分地点仍复用背景和布局；探索物、道具及常见NPC均属于网页独立表现设计。
...
以下按发生时间保留各轮发现；当时的素材来源缺口不等于当前发布目录状态。2026-09-24核验的当前31张PNG均有制作记录，文件哈希、文档和输入依赖检查通过；`DOCUMENTED_RECORDS_ONLY`只证明记录一致，不构成法律原创、权属或授权认证。原有缺口及替换过程继续保留在[资源边界审计](docs/reference-boundary-audit.md)、[原创人物记录](docs/characters-original-art.md)和[原创环境记录](docs/landscape-original-art.md)。
...
资源清单位于 docs/asset-provenance.json；npm run test:assets 核对图片与记录的哈希、文件集合、文档路径及参考输入依赖。部署与部署预检会先执行该检查。当前输出为31项 documented、无清单内未确认项；历史的六项旧素材与一项上游依赖缺口保留在原审计记录，不能因后来替换而改写其历史来源。检查通过仍只表示当前文件与记录一致。
