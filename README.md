...
目前包含 168 个剧情事件、89 个现实地点、18 门武学和 7 个结局。剧情按攻略与本地只读参考分阶段核对，衔接对白独立编写；已验证范围见审计文档，不表示原版逐句一致。场景映射使用 18 张环境图（16 张现实环境、2 张临时梦境），部分地点复用背景和布局；新增的探索物、道具及八类常见 NPC 属于网页重建设计。
...
本轮禁地逐层追踪、两玉开门、密室读信揭面与主动夜访的独立实现和实测，见 [禁地试玩](docs/forbidden-browser.md) 与 [夜访试玩](docs/night-visit-browser.md)。新增底图完全从原创文字生成，来源及完整提示词见 [制作记录](docs/forbidden-art.md)。原版目录只读参考，不复制原版素材、脚本、完整台词或解包副本；实现与资源均由本项目独立制作。
