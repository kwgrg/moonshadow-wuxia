...
本轮补充小筑醒后返庄传讯，以及正线码头急报、山庄解围、传功和回小筑寻人。实际浏览器记录见 [返庄验收](docs/hut-return-browser.md) 与 [正线验收](docs/valley-defense-browser.md)。它是局部实现增量，不代表全流程复刻完成。
...
当前版本为存档修订16；当前推进范围与未完成项目以 [全流程完成清单](docs/fullflow-completion.md) 为准。任务和场景数量不代表原作覆盖率。剧情按攻略与本地只读参考分阶段核对，衔接对白独立编写；已验证范围见审计文档，不表示原版逐句一致。部分地点仍复用背景和布局；探索物、道具及常见NPC均属于网页独立表现设计。
...
发布入口已固定为 `scripts/deploy.mjs`：先检查当前Git受控与未忽略文件的格式、完整 `public/` 文件树和40项图片来源记录，再调用项目锁定的Wrangler。未确认来源会阻断发布；额外的目录、配置或环境覆盖参数会被拒绝。新增媒体格式须先扩展来源检查，不能直接放入其他发布子目录。检查只核对文件边界与记录一致性，不判断全文表达相似性或证明授权；人工审查仍遵循 [项目约束](AGENTS.md)。本轮范围与验证见 [来源门禁记录](docs/reference-publication-guard.md)。
...
以下按发生时间保留各轮发现；当时的素材来源缺口不等于当前发布目录状态。2026-09-24核验的当前40张PNG均有制作记录，文件哈希、文档和输入依赖检查通过；`DOCUMENTED_RECORDS_ONLY`只证明记录一致，不构成法律原创、权属或授权认证。原有缺口及替换过程继续保留在[资源边界审计](docs/reference-boundary-audit.md)、[原创人物记录](docs/characters-original-art.md)和[原创环境记录](docs/landscape-original-art.md)。

已把旅行改为沿相邻地图出口逐段行走，新增两段山路并接通四个支线区域；祭父、酒肆旁听和洗剑池对峙现在包含人物走位与可续存的演出。当时主角跪姿由旧人物图生成，上游直接制作记录待补齐；这组旧发布文件已在后续轮次替换，历史缺口未被追认成已核来源。

后段补入婚宴三场单挑、庄外蒙面人交手、小筑单挑，以及夜袭前授技；蔷薇护送回谷后才授太极剑谱，十二株银丝草需要实际返屋交付，玉佩与书信按发生顺序入账。当时小筑揭露紫轩身份的时点与参考记录有冲突，已由后续邪线地牢修订改为战前认出；其余未完成事项仍按清单保留。
...
2026-09-22：邪线新增禁地逆向返程后的楼中插叙、村民求援与36敌岛战、五居民送别、真实航渡和两种码头结果。月眉儿使用新作立绘，两个地点使用独立环境图；具体来源与局限见 [本轮美术](docs/evil-docks-art.md)，参考边界复核见 [资源来源审计](docs/reference-boundary-audit.md)。这一轮当时有六项旧图缺直接制作记录，作为历史待确认项保留；没有据此认定它们来自原版。后续版本已从发布目录移除这些旧图并换用有直接制作记录的资源，原始缺口并未被抹去。

资源清单位于 docs/asset-provenance.json；npm run test:assets 核对图片与记录的哈希、文件集合、文档路径及参考输入依赖。部署与部署预检会先执行该检查。当前输出为40项 documented、无清单内未确认项；历史的六项旧素材与一项上游依赖缺口保留在原审计记录，不能因后来替换而改写其历史来源。检查通过仍只表示当前文件与记录一致。
