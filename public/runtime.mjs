import {migrateGoodMedicineReunion,repairMedicineBranch} from './good-medicine-reunion-migration.mjs';
import {firstMeetingMethods,restoreFirstMeeting} from './first-meeting-runtime.mjs';
import {towerMethods,restoreTowerJourney} from './tower-runtime.mjs';
import {migrateGoodTowerValley} from './good-tower-valley-migration.mjs';
import {restoreCombatProgress,combatMethods,isOrdinaryCombat} from './combat-progress.mjs';
...
import {partyMethods,applyCompanionEffects,normalizeCompanions} from './travel-party.mjs';
...
import { QUESTS, MAPS, SKILLS, ITEMS, ENDINGS, SIDE_QUESTS, chooseEnding, LEGACY_QUEST_IDS, REVISION_TWO_QUEST_IDS, REVISION_THREE_QUEST_IDS, REVISION_FOUR_QUEST_IDS, REVISION_FIVE_QUEST_IDS, REVISION_SIX_QUEST_IDS, REVISION_SEVEN_QUEST_IDS, REVISION_EIGHT_QUEST_IDS, REVISION_NINE_QUEST_IDS, REVISION_TEN_QUEST_IDS, REVISION_ELEVEN_QUEST_IDS, REVISION_TWELVE_QUEST_IDS, REVISION_THIRTEEN_QUEST_IDS, REVISION_FOURTEEN_QUEST_IDS, REVISION_FIFTEEN_QUEST_IDS } from './campaign.mjs';
...
export function restoreState(raw){
 ...
 if(Array.isArray(raw.flags?.companions)){const members=normalizeCompanions(raw.flags.companions);if(!Object.hasOwn(raw.flags,'companion')||raw.flags.companion===(members[0]||null))applyCompanionEffects(s.flags,{companions:members});}
 ...
 raw=migrateGoodTowerValley(raw,s,QUESTS,REVISION_FOURTEEN_QUEST_IDS);
 raw=migrateGoodMedicineReunion(raw,s,QUESTS,REVISION_FIFTEEN_QUEST_IDS);
 ...
 restoreCombatProgress(raw,QUESTS[s.quest],s);
 restoreTowerJourney(raw,s);
 restoreFirstMeeting(QUESTS[s.quest],s);
...
export class GameEngine{
 ...
 get scene(){
  const medicineChapter=this.s.flags.route==='good'&&!this.s.flags.forsake&&!this.s.flags.cultPath&&/^g2[1-4](?:_|$)/.test(this.q.id);
  const medicineCue=this.s.sequence?.cues.valleyCareLight;
  const medicineNight=medicineCue?medicineCue==='night':!!(this.s.flags.goodMedicineCured||['g22','g22_rest'].includes(this.q.id))&&!this.s.flags.goodMedicineMorningReady&&!this.s.flags.staged_g22_rest;
  const dream=this.stagingScene();if(dream)return medicineChapter?{...dream,atmosphere:{...dream.atmosphere,light:medicineNight?'night':'day'}}:dream;
  const aftermath=this.s.flags.route==='good'&&(this.q.id==='g19_return'||this.s.flags.goodTowerHomecoming||this.s.flags.goodTowerLegacyNight||this.s.flags.goodTowerValleyLegacy);
  const medicine=medicineChapter&&['m23','m49','m50','m17','m16','m70','m41','r_good_hanbo_road','r_good_manor_infirmary','r_good_manor_zhen_room',...(this.q.id==='g24_departure'?['m34']:[])].includes(this.s.map);
  let scene=getScene(this.s.map,this.region,medicine?'medicineCare':aftermath?'towerAftermath':undefined);
  if(this.s.map==='r_leaf_memorial'){const cues=this.stagingPresentation()?.cues||{},props=scene.props.filter(p=>!p.requireFlag||this.s.flags[p.requireFlag]||(p.requireFlag==='goodTowerMengBuried'&&this.s.flags.goodTowerLegacyMengBuried)||cues[p.requireCue?.key]===p.requireCue?.value);scene={...scene,props,obstacles:[...scene.obstacles,...props.filter(p=>p.footprint).map(p=>p.footprint)]};}
  if(aftermath&&['m51','r_leaf_memorial','r_leaf_hero_room','r_leaf_rose_room'].includes(this.s.map)){const cue=this.s.sequence?.cues.valleyCareLight,night=cue?cue==='night':!!(this.s.flags.goodRoseNightReady||this.s.flags.goodTowerLegacyNight)&&!this.s.flags.goodRoseNightComplete&&!this.s.flags.goodRoseDead&&!this.s.flags.goodRoseAloneMorning;scene={...scene,atmosphere:{...scene.atmosphere,light:night?'night':'day'}};}
  ...
  if(aftermath&&['m51','r_leaf_memorial','r_leaf_hero_room','r_leaf_rose_room'].includes(this.s.map))return scene;
  if(medicine)return {...scene,atmosphere:{...scene.atmosphere,light:medicineNight?'night':'day'}};
 ...
 get markers(){
  ...
  if(at&&!this.s.completed){
   if(q.firstMeeting)list.push(...this.firstMeetingMarkers());
   else if(q.rescueMission){const marker=this.rescueMarker();if(marker)list.push(marker);}
   ...
   else if(this.canStartStaging()){const definition=this.stagingDefinition(),point=definition.startPoint||definition.trigger||definition.heroStart||scene.objective;list.push({id:'staging-start',kind:'main',paintOnly:!!definition.pointOnly,x:point.x,y:point.y,name:definition.label||(q.id==='a03'?'池边石碑':q.id==='a01'?'辞别父亲':'走近酒肆'),main:true,sprite:null});}
   else if(!q.towerPassage&&!['battle','training','staging'].includes(this.s.phase))list.push({id:'main',kind:'main',x:q.x??scene.objective.x,y:q.y??scene.objective.y,name:(this.s.phase==='choice'&&q.choiceSpeaker)||(revealed&&q.stagedNpc)||q.npc||q.object||'江湖纪事',main:true,sprite:this.s.phase==='choice'?(q.choiceSprite??q.sprite):revealed?(q.stagedSprite??q.sprite):q.sprite,...(this.s.phase==='after'?q.afterMarker||{}:{})});
  }
 ...
 interact(marker=null){if(this.paused||this.s.phase==='staging'||this.jump)return false;const m=marker||this.markers.filter(m=>m.main).sort((a,b)=>distance(this.s.hero,a)-distance(this.s.hero,b))[0]||this.npc;if(!m||m.interactive===false||m.hidden||m.pose==='fallen')return false;
  if(this.s.completed&&!['travel','inspect','chest'].includes(m.kind)){this.emit('ending');return true;}if(this.s.phase==='battle'&&!(this.towerCanTravel()&&['travel','inspect','chest','side'].includes(m.kind))&&!(m.kind==='rescue'&&this.canStartRescueStaging())&&!(m.kind==='travel'&&this.rescueReady()&&m.to===this.q.rescueMission.exitMap)){this.emit('toast',{text:'先解决眼前的对手。'});return false;}
  ...
  if(m.kind==='firstMeeting')return this.interactFirstMeeting(m);
 ...
 beginObjective(){if(this.s.failure||(this.s.skirmish?.failed||this.s.combatProgress?.failed)||this.s.map!==this.q.map)return;if(!this.requireQuestItems()||!this.requireQuestFlags())return;if(this.q.rescueMission){this.ensureRescueEncounter();if(this.canStartRescueStaging())this.startStaging();return;}if(this.canStartStaging()){this.startStaging();return;}if(this.s.sequence)return;const q=this.q;if(q.firstMeeting){this.emit('toast',{text:'走近想先交谈的人，再开口。'});return;}if(q.towerPassage){this.trackTower();return;}if(q.pursuit){this.startPursuit();return;}if(q.battleBeforeChoice&&this.s.phase==='choice'){this.emit('choice');return;}if(q.training){this.ensureTraining();this.s.phase='training';this.emit('objective');return;}if(q.choiceBeforeObjective&&!Object.hasOwn(this.s.choices,q.id)){this.s.phase='choice';this.emit('choice');return;}if(q.battleBeforeChoice){this.startBattle();return;}if(q.type==='battle'||q.type==='boss'){this.startBattle();return;}if(q.type==='escape'){this.s.phase='escape';this.s.timer=q.duration;this.emit('objective');return;}if(['search','fetch','puzzle'].includes(q.type)){this.s.collected=0;this.s.collectedIds=[];this.s.timer=q.timeLimit||0;this.s.phase='search';this.emit('objective');return;}if(q.type==='choice'||q.choice){this.s.phase='choice';this.emit('choice');return;}this.completeQuest();}
 ...
 startBattle(nextWave=false){
  if(this.s.failure||(this.s.skirmish?.failed||this.s.combatProgress?.failed))return false;
  if(this.towerEncounter())return this.ensureTowerEncounter();
  if(this.q.skirmish)return this.startSkirmish();
  if(!isOrdinaryCombat(this.q)||this.s.hero.hp<=0)return false;
  if(this.resumeCombatEncounter())return true;
 ...
 choose(index){
  const q=this.q,options=q.choice?.options;if(q.firstMeeting)return false;
  if(this.s.map!==q.map||this.s.phase!=='choice'||this.s.sequence||this.s.completed||this.s.failure||(this.s.skirmish?.failed||this.s.combatProgress?.failed)||this.s.done.includes(q.id)||!options||!Number.isInteger(index)||!options[index])return false;
  const answer=recordedChoice(q,this.s),resuming=singleChoice(q)&&answer!==null;
  if(resuming&&answer!==index)return false;
  if(q.battleBeforeChoice&&!this.canCompleteCombat())return false;
  ...
  this.s.choices[this.q.id]=index;if(this.resolveRefusal(index))return true;if(q.choiceRestoreStaticFlag&&this.s.flags[q.choiceRestoreStaticFlag])repairChoiceAssignments(q,this.s,index);else this.applyEffects(choice.effects);if(this.q.repeatRefusal&&index===1){this.s.flags.refusals=(this.s.flags.refusals||0)+1;if(this.s.flags.refusals<4){this.emit('choice');return true;}this.s.flags.forsake=true;}if(choice.ending){this.finish(choice.ending);return true;}if(this.q.choiceBeforeObjective){this.beginObjective();return true;}this.completeQuest();return true;}
 completeQuest(){
  if(this.s.failure||(this.s.skirmish?.failed||this.s.combatProgress?.failed)||this.s.completed||this.s.sequence||this.s.map!==this.q.map)return;const q=this.q;if(q.firstMeeting&&(this._firstMeetingFinishCommit!==q.id||!this.firstMeetingReady()))return;if(q.towerPassage&&this._towerExitCommit!==q.id)return;if(isOrdinaryCombat(q)&&!this.canCompleteCombat())return;if(singleChoice(q)&&!choiceFlagsMatch(q,this.s))return;if(q.id==='e09'){const answer=this.s.choices.e09;if(!this.s.flags.evilManorDecision||![0,1].includes(answer)||this.s.flags.evilMeiEscorted!==(answer===0)||this.s.flags.evilMeiAlone!==(answer===1))return;}if(q.pursuit&&(!this.s.pursuit?.finished||this.s.pursuit.questId!==q.id||this.s.phase!=='pursuit'))return;if(q.rescueMission&&(this._rescueExitCommit!==q.id||!this.rescueReady()))return;if(q.skirmish&&!q.rescueMission&&(!this.s.skirmish?.finished||this.s.phase!=='after'))return;if(q.refusalRule&&(this.s.phase!=='choice'||!Object.hasOwn(this.s.choices,q.id)||(this.s.choices[q.id]===(q.refusalRule.refuseIndex??1)&&this.refusalCount()<q.refusalRule.limit)))return;if(q.requireStaging&&!this.s.flags['staged_'+q.id]&&!this.s.flags[q.legacyStagingFlag]&&!this.s.done.includes(q.id))return;if(!this.requireQuestItems()||(!this.s.claimedRewards?.includes(q.id)&&!this.requireItems(this.outstandingItems(q.consumeItems))))return;if(q.training&&this.s.phase!=='after'){this.emit('toast',{text:'本次试剑尚未结束。'});return;}if(!this.requireQuestFlags())return;if(q.ending&&this.s.flags.route==='good'&&!this.s.flags.cultPath&&!repairMedicineBranch(this.s,q.id)){this.recoverUnknownFirstMeeting(q.id);return;}if(this.s.done.includes(q.id))return;this.s.done.push(q.id);const claimed=this.s.claimedRewards??=[];const firstReward=!claimed.includes(q.id);if(firstReward){claimed.push(q.id);for(const [id,count] of Object.entries(this.outstandingItems(q.consumeItems)))this.s.inventory[id]-=count;this.applyEffects(q.rewards);for(const [id,count] of Object.entries(q.ensureItems||{}))if(ITEMS[id])this.s.inventory[id]=Math.max(this.s.inventory[id]||0,count);this.gainExp(q.xp??65);this.s.coins+=q.money??15;if(!q.suppressBattleSupplies&&(q.type==='battle'||q.type==='boss')){this.s.potions++;this.s.elixirs++;}}else{for(const [key,value] of Object.entries(q.rewards?.flags||{}))this.s.flags[key]=value;if(q.rescueMission||q.repairCompanions)applyCompanionEffects(this.s.flags,q.rewards||{});}
  ...
  this.s.objectiveProgress=null;this.s.pursuit=null;this._pursuitPath=null;this._pursuitFollow=false;this.s.destination=null;this.s.allies=[];this.s.skirmish=null;this.s.combatProgress=null;this.s.quest++;while(QUESTS[this.s.quest]&&((QUESTS[this.s.quest].when&&!this.matches(QUESTS[this.s.quest].when))||this.s.done.includes(QUESTS[this.s.quest].id))){this.s.quest++;}
  if(this.s.quest>=QUESTS.length){this.s.quest=QUESTS.length-1;this.finish(chooseEnding(this.s));return;}
  this.s.phase=this.s.map===this.q.map?'talk':'travel';this.s.collected=0;this.s.collectedIds=[];this.s.enemies=[];this.target=null;this.waypoints=[];this.autoInteract=null;if(q.transition?.map&&MAPS[q.transition.map]){this.s.map=q.transition.map;if(!this.s.visited.includes(this.s.map))this.s.visited.push(this.s.map);this.s.phase=this.s.map===this.q.map?'talk':'travel';Object.assign(this.s.hero,this.scene.spawn);this.resetParty();this.emit('chapter');}else this.emit('quest');
 }
 ...
 finish(id){if(!ENDINGS[id])return false;this.s.ending=id;this.s.completed=true;this.s.phase='complete';this.s.flags.companion=null;this.emit('ending');}
 ...
 travelBlocked(){return !!(this.q.firstMeeting&&this.s.map===this.q.map&&this.firstMeetingReady())||!!this.jump||!!this.s.failure||this.paused||(this.s.phase==='battle'&&!this.rescueReady()&&!this.towerCanTravel())||['escape','staging'].includes(this.s.phase)||(['search','return'].includes(this.s.phase)&&this.q.timeLimit);}
 travel(id){
 ...
 enterMap(id){
  ...
  const leavingTower=!!this.towerEncounter();if(leavingTower&&!this.towerExitReady(id))return false;
  ...
  this.resetParty();this.jump=null;this.autoInteract=null;this.attackTarget=null;this.effects=[];this.s.enemies=[];if(leavingTower){this.s.allies=[];this.s.skirmish=null;}this.ensureTowerEncounter();this._portalCooldown=.8;
 ...
 cast(index){const skill=SKILLS[index],h=this.s.hero;if(!skill||h.hp<=0||this.s.combatProgress?.failed||this.jump||this.paused||!this.active||this.s.completed||this.s.phase==='staging')return false;if(index!==0&&!Object.hasOwn(this.s.skills,index)){this.emit('toast',{text:'这门武学尚未习得。'});return false;}if(this.s.cooldowns[index]>0)return false;if(h.mp<skill.cost){this.emit('toast',{text:'内力不足，可用补气丹或在安全处打坐。'});return false;}if(skill.heal&&h.hp>=h.maxHp)return false;
  ...
  for(const e of sorted){if(distance(h,e)>skill.range)continue;if(index===0&&hit)break;const damage=Math.round((skill.damage+this.damageBonus())*(1+(h.level-1)*.055)*(1+(level-1)*.08));e.hp=Math.max(this.encounter.forcedOutcome==='defeat'?1:0,e.hp-damage);e.flash=.2;this.addNumber(damage,e.x,e.y-90,skill.color);hit++;if(skill.slow)e.slow=3;if(e.hp===0){if(this.battleQuest.skirmish){this.markSkirmishDefeat(e,true);continue;}const receipt=this.claimCombatDefeat(e);if(receipt){this.s.kills+=receipt.kills;this.s.coins+=receipt.coins;this.gainExp(receipt.xp);}this.emit('combatProgress');this.addEffect('spark',e.x,e.y-40,55,'#eac98a',.8);}}
  if(this.battleQuest.skirmish){this.checkSkirmishOutcome();return true;}
  if(this.s.phase==='battle'&&this.s.enemies.every(e=>e.hp<=0)){if(!this.finishCombatProgress('victory')){if(this.s.combatProgress?.failed)this.emit('defeat');return true;}if(this.q.training&&this.ensureTraining().active!==null){this.finishTrainingWin();return true;}if(this.q.waves&&(this.s.wave||0)<this.q.waves.length-1){this.s.wave=(this.s.wave||0)+1;h.hp=Math.min(h.maxHp,h.hp+100);h.mp=Math.min(h.maxMp,h.mp+60);this.startBattle(true);this.emit('wave',{index:this.s.wave+1,total:this.q.waves.length,name:this.encounter.boss||this.encounter.enemy});return true;}this.s.phase='after';this.attackTarget=null;this.target=null;this.waypoints=[];if(this.q.openingEnding)this.finish('opening');else if(this.q.battleBeforeChoice){this.s.phase='choice';this.emit('choice');}else this.emit('victory');}return true;
 }
 ...
 potion(){if(this.s.failure||(this.s.skirmish?.failed||this.s.combatProgress?.failed))return false;if(this.s.potions<=0){this.emit('toast',{text:'金创药已用尽，可拜访商人补充。'});return false;}const h=this.s.hero;if(h.hp>=h.maxHp){this.emit('toast',{text:'气血充盈。'});return false;}const n=Math.min(h.maxHp-h.hp,Math.max(180,h.maxHp*.48));h.hp+=n;this.s.potions--;this.addNumber('+'+Math.round(n),h.x,h.y-90,'#c8e5af');return true;}
 elixir(){if(this.s.failure||(this.s.skirmish?.failed||this.s.combatProgress?.failed))return false;if(this.s.elixirs<=0){this.emit('toast',{text:'补气丹已用尽，可在安全处按 V 打坐。'});return false;}const h=this.s.hero;if(h.mp>=h.maxMp)return false;h.mp=Math.min(h.maxMp,h.mp+Math.max(120,h.maxMp*.6));this.s.elixirs--;return true;}
 ...
 meditate(){if(this.jump||this.paused||this.s.phase==='staging')return;if(this.s.phase==='battle'){this.emit('toast',{text:'交战中无法打坐。'});return;}this.meditating=!this.meditating;this.target=null;this.waypoints=[];if(this.meditating){this.s.destination=null;this.autoInteract=null;this.attackTarget=null;}this.emit('toast',{text:this.meditating?'凝神静气，恢复气血与内力。':'收功起身。'});}
 tick(dt){dt=clamp(dt,0,.05);if(this.sceneLoading||this.s.failure||this.s.combatProgress?.failed||this.paused||!this.active)return;this.time+=dt;this.s.playTime+=dt;if(this.jump){this.tickJump(dt);return;}this.ensureTowerEncounter();this.ensureRescueEncounter();if(this.canStartStaging()){const d=this.stagingDefinition();if(this.hasQuestItems()&&(d.auto||(d.trigger&&distance(this.s.hero,d.trigger)<(d.trigger.radius||135))))this.startStaging();}if(this.s.phase==='staging'){this.tickStaging(dt);return;}this._portalCooldown=Math.max(0,(this._portalCooldown||0)-dt);this.dashTime=Math.max(0,this.dashTime-dt);this.hitTime=Math.max(0,this.hitTime-dt);this.s.cooldowns=this.s.cooldowns.map(c=>Math.max(0,c-dt));this.s.flags.shield=Math.max(0,(this.s.flags.shield||0)-dt);const h=this.s.hero;h.stamina=Math.min(100,h.stamina+dt*12);h.mp=Math.min(h.maxMp,h.mp+dt*(this.meditating?35:this.s.phase==='battle'?2:6));if(this.meditating)h.hp=Math.min(h.maxHp,h.hp+dt*30);
  ...
  this.stepParty(dt);this.tickTowerEscort(dt);
  ...
  if(this.s.phase==='battle'&&this.battleQuest.skirmish)this.tickSkirmish(dt);
   ...
   }if(h.hp<=0){if(this.encounter.scriptedLoss){if(!this.finishCombatProgress('scripted-loss')){this.failCombatProgress('incomplete-roster');this.emit('defeat');return;}h.hp=this.q.training?h.maxHp:Math.round(h.maxHp*.25);if(this.q.training)this.ensureTraining().finished=true;this.s.enemies=[];this.s.phase=this.q.battleBeforeChoice?'choice':'after';this.emit('scriptedLoss');if(this.q.battleBeforeChoice)this.emit('choice');}else{this.failCombatProgress('hero');this.emit('defeat');}}
  }
 ...
 retry(){if(this.towerEncounter())return this.retryTower();if(this.s.failure||(!this.s.skirmish?.failed&&!this.s.combatProgress?.failed&&this.s.hero.hp>0))return false;this.resetRescueAttempt();if(isOrdinaryCombat(this.q))this.resetCombatAttempt();if(this.q.skirmish)this.s.skirmish=null;this.s.hero.hp=this.s.hero.maxHp;this.s.hero.mp=this.s.hero.maxMp;this.s.hero.stamina=100;this.s.hero.x=this.scene.spawn.x;this.s.hero.y=this.scene.spawn.y;this.s.cooldowns=Array(SKILLS.length).fill(0);this.paused=false;this.startBattle();}
}

Object.assign(GameEngine.prototype,firstMeetingMethods,towerMethods,combatMethods,stagingMethods,recruitmentMethods,skirmishMethods,pursuitMethods,rescueMethods);
